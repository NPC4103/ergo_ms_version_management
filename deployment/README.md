...
### 7. **create** — создать новый репозиторий через API
   ...
   - **Примечание:** Работает через API, требует запущенный сервер

### 8. **download** — скачать сторонний репозиторий
...
- `--description <текст>` — **игнорируется** (API не поддерживает описание)
- `--root <путь>` — **игнорируется** (работает через API, не требует локального корня проекта)
- **Примечание:** Команда работает через API, требует запущенный сервер
